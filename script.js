...
const assetImages = {
  ...
  products: {
    'nakshi-dream-frame': 'assets/images/products/nakshi-dream-frame.png',
    'golden-river-canvas': 'assets/images/products/golden-river-canvas.png',
...
{
  id:'golden-river-canvas', name:'Nivarpon - The Offering of a Fading Flame', category:'Painting', price:6200, artistId:'mrinmoy-dey', featured:true,
  story:'A fading flame reaching toward another, inspired by the quiet moments where support is felt but rarely seen.',
  description:'This painting captures a deeply human moment — where one presence gently offers light to another. It reflects the silent support we receive in life, often unnoticed yet profoundly felt.',
  longDescription:'Through the imagery of two flames, the artwork explores the idea that no one moves forward alone. Even in our darkest moments, there is always someone who stands beside us, guiding us without words. The fading light does not disappear; instead, it gives itself to sustain another, symbolizing care, dependence, and the quiet strength of human connection.',
  ...
  making:['Concept sketch','Base flame composition','Light balance and emotional tone','Final sealing'], certificate:'KC-GRC-2026-002', edition:'Studio Selection', authenticity:'Numbered authenticity card included',
  featuredQuote:'Even a fading light can guide another.', video:'assets/videos/golden-river-canvas.mp4'
},
  {
...
function renderHome(){
  ...
  if(hv){ const src=assetImages.videos.homeStudioFilm || ''; hv.innerHTML = `<div class="video-panel"><div class="video-frame">${src ? `<video class="video-placeholder" controls playsinline preload="metadata" src="${src}"></video>` : `<div class="video-placeholder"><div class="play-badge">▶</div></div>`}</div><div class="video-meta"><div><span class="eyebrow">Studio Film</span><h3 style="font-family:Georgia,serif;font-size:34px;margin:12px 0 8px">Inside the making of Bangladeshi craft</h3><p class="muted">Watch the hands, materials, and rhythms behind the works featured on KaruCart.</p></div><a class="btn btn-secondary" href="artworks.html">Explore collection</a></div></div>`; }
}
...
<div>
    <div class="detail-visual" data-title="${p.name}"${bgStyle(productImage(p.id))}></div>
  </div>
  <div><span class="eyebrow">${p.category}</span><h1>${p.name}</h1><p>${p.description}</p>
    ...
    const src = p.video || '';
    byId('productVideo').innerHTML = `<div class="section-head"><div><span class="eyebrow">Video Story</span><h2>See the artwork in motion</h2><p class="muted">See the piece in context through a process film, studio view, or interior presentation.</p></div></div><div class="video-panel"><div class="video-frame"><div class="inline-video ${src ? '' : 'fallback'}">${src ? `<video controls playsinline preload="metadata" src="${src}"></video>` : `<div class="play-badge">▶</div>`}</div></div><div class="video-meta"><div><h3 style="font-family:Georgia,serif;font-size:30px;margin:0 0 8px">${p.name} film section</h3><p class="muted">This film offers a closer look at the making process, final detailing, or how the piece lives inside a space.</p></div><a class="btn btn-secondary" href="certificate.html?id=${p.id}">View certificate</a></div></div>`;
  }
